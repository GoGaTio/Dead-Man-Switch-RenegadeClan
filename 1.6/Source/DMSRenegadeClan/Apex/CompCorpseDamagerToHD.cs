...
namespace AnimalBehaviours
{
    public class CompCorpseDamagerToHD : ThingComp
    {
        ...
        public override void CompTick()
        {
			...
			if (tickCounter > Props.tickInterval)
			{
				...
				if (pawn.Map != null)
				{
					CellRect rect = GenAdj.OccupiedRect(pawn.Position, pawn.Rotation, IntVec2.One).ExpandedBy(Props.radius);
						...
						if (current.InBounds(pawn.Map))
						{
							...
							if (hashSet != null)
							{
								foreach (Thing thingInCell in hashSet)
								{
									...
									if (corpse != null && !(corpse.IsDessicated() && Props.notOnDessicated))
									{
										...
										if (corpse.HitPoints < 0)
										{
											...
											{
												IntVec3 c;
												RCellFinder.TryFindRandomReachableCellNear(pawn.Position, pawn.Map, 2, TraverseParms.For(TraverseMode.NoPassClosedDoors, Danger.Deadly, false), null, null, out c);
	...
	public class CompProperties_CorpseDamagerToHD : CompProperties
    {
		...
		public bool notOnDessicated = true;
